  ...
  return <div className="screen-form narrow">
    ...
    <Panel className="block-panel">
      ...
      <div className="settings-panel compact">
        ...
        <Toggle label="Chặn mọi loại file (PDF, slide…)" checked={allFiles} onChange={setAllFiles} />
    ...
    <button className="primary-action" disabled={busy || !ownerEmail || !folders.trim()} onClick={() => onSubmit({ owner_email: ownerEmail, folders: folders.split(/\r?\n|,/).map(v => v.trim()).filter(Boolean), recursive, all_files: allFiles, workers: Number(workers), unblock, dry_run: dryRun })}><CirclePlay size={19} />{busy ? 'Đang xử lý…' : dryRun ? 'Chạy thử' : unblock ? 'Áp dụng cho phép tải' : 'Áp dụng chặn tải'}</button>
  </div>
}
